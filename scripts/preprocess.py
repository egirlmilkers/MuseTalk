	...
	for idx, vid in enumerate(vid_list):
		if vid.endswith('.mp4'):
			...
			audio_output_path = os.path.join(
				dst_path, os.path.splitext(vid)[0] + '.wav'
			)
			try:
				command = [
					'ffmpeg',
...
def main(cfg):
	...
	segment_video(
		cfg.video_root_25fps,
		cfg.video_audio_clip_root,
		vid_list,
		segment_duration=cfg.clip_len_second,
	)
...
if __name__ == '__main__':
	...
	parser.add_argument(
		'--config', type=str, default='./configs/training/preprocess.yaml'
	)
