...
def main(args):
	...
	if not fast_check_ffmpeg():
		...
		if not fast_check_ffmpeg():
			print(
				'Warning: Unable to find ffmpeg, please ensure ffmpeg is properly installed'
			)
	...
	for task_id in inference_config:
		try:
			...
			if args.version == 'v15':
				...
				bbox_shift = inference_config[task_id].get(
					'bbox_shift', args.bbox_shift
				)  # v1 uses config or default
			...
			crop_coord_save_path = os.path.join(
				args.result_dir, '../', input_basename + '.pkl'
			)
			...
			_ = os.path.join(
				temp_dir, output_basename + '_concat.mp4'
			)  # output_vid_name_concat
			...
			for i, res_frame in enumerate(tqdm(res_frame_list)):
				...
				if args.version == 'v15':
					...
					combine_frame = get_image(
						ori_frame, res_frame, [x1, y1, x2, y2], fp=fp
					)
				cv2.imwrite(
...
if __name__ == '__main__':
	...
	parser.add_argument(
		'--ffmpeg_path',
		type=str,
		default='./ffmpeg-4.4-amd64-static/',
		help='Path to ffmpeg executable',
	)
	parser.add_argument('--gpu_id', type=int, default=0, help='GPU ID to use')
	parser.add_argument(
		'--vae_type', type=str, default='sd-vae', help='Type of VAE model'
	)
	parser.add_argument(
		'--unet_config',
		type=str,
		default='./models/musetalk/config.json',
	...
	parser.add_argument(
		'--inference_config',
		type=str,
		default='configs/inference/test_img.yaml',
		help='Path to inference configuration file',
	)
	parser.add_argument(
		'--bbox_shift', type=int, default=0, help='Bounding box shift value'
	)
	parser.add_argument(
		'--result_dir', default='./results', help='Directory for output results'
	)
	parser.add_argument(
		'--extra_margin', type=int, default=10, help='Extra margin for face cropping'
	)
	...
	parser.add_argument(
		...
		type=int,
		default=2,
		help='Left padding length for audio',
	)
	parser.add_argument(
		'--audio_padding_length_right',
		type=int,
		default=2,
		help='Right padding length for audio',
	)
	parser.add_argument(
		'--batch_size', type=int, default=8, help='Batch size for inference'
	)
	parser.add_argument(
		'--output_vid_name', type=str, default=None, help='Name of output video file'
	)
	parser.add_argument(
		'--use_saved_coord',
		action='store_true',
		help='Use saved coordinates to save time',
	)
	parser.add_argument(
		'--saved_coord', action='store_true', help='Save coordinates for future use'
	)
	parser.add_argument(
		'--use_float16', action='store_true', help='Use float16 for faster inference'
	)
	parser.add_argument(
		'--parsing_mode', default='jaw', help='Face blending parsing mode'
	)
	parser.add_argument(
		'--left_cheek_width', type=int, default=90, help='Width of left cheek region'
	)
	parser.add_argument(
		'--right_cheek_width', type=int, default=90, help='Width of right cheek region'
	)
	parser.add_argument(
		'--version',
		type=str,
