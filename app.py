...
def debug_inpainting(
	...
	fp = FaceParsing(
		left_cheek_width=args.left_cheek_width, right_cheek_width=args.right_cheek_width
	)
	...
	pred_latents = unet.model(
		latents, timesteps, encoder_hidden_states=audio_feature
	).sample
	...
	combine_frame = get_image(
		frame, res_frame, [x1, y1, x2, y2], mode=args.parsing_mode, fp=fp
	)
...
def inference(
	...
	# Save video
	imageio.mimwrite(
		output_video, images, 'FFMPEG', fps=fps, codec='libx264', pixelformat='yuv420p'
	)
	...
	with gr.Row():
		with gr.Column():
			...
			extra_margin = gr.Slider(
				label='Extra Margin', minimum=0, maximum=40, value=10, step=1
			)
			parsing_mode = gr.Radio(
				label='Parsing Mode', choices=['jaw', 'raw'], value='jaw'
			)
			left_cheek_width = gr.Slider(
				label='Left Cheek Width', minimum=20, maximum=160, value=90, step=5
			)
			right_cheek_width = gr.Slider(
				label='Right Cheek Width', minimum=20, maximum=160, value=90, step=5
			)
...
demo.queue().launch(
	share=args.share, debug=True, server_name=args.ip, server_port=args.port
)
