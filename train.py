...
def main(cfg):
	...
	loss_dict = initialize_loss_functions(
	...
	(
		...
		model_dict['lr_scheduler'],
		dataloader_dict['train_dataloader'],
	...
	print(
		'length train/val',
		len(dataloader_dict['train_dataloader']),
		len(dataloader_dict['val_dataloader']),
	)
	...
	for epoch in range(first_epoch, num_train_epochs):
		...
		for step, batch in enumerate(dataloader_dict['train_dataloader']):
			...
			with torch.no_grad():
				...
				audio_prompts = process_audio_features(
					cfg, batch, model_dict['wav2vec'], bsz, num_frames, weight_dtype
				)
				...
				if cfg.loss_params.sync_loss > 0:
					...
					if cfg.use_adapted_weight:
						...
						image_audio_sim_gt = F.cosine_similarity(
							audio_embed, vision_embed_gt, dim=1
						)[0]
						...
						if image_audio_sim_gt < 0.05 or image_audio_sim_gt > 0.65:
							if cfg.adapted_weight_type == 'cut_off':
								...
								print(
									f'unknown adapted_weight_type: {cfg.adapted_weight_type}'
								)
								adapted_weight = 1
					...
					frames_left_index = (
						random.randint(0, max_start) if max_start > 0 else 0
					)
				...
				pixel_values_backward = pixel_values[
					:, frames_left_index:frames_right_index, ...
				]
				...
				masked_latents = (
					model_dict['vae'].encode(masked_frames).latent_dist.mode()
				)
				masked_latents = (
					masked_latents * model_dict['vae'].config.scaling_factor
				)
				masked_latents = masked_latents.float()

				# Encode reference images
				ref_frames = rearrange(
					ref_pixel_values_backward, 'b f c h w-> (b f) c h w'
				)
				...
				pixel_values_face_mask_backward = rearrange(
					pixel_values_face_mask_backward, 'b f c h w -> (b f) c h w'
				)
				audio_prompts_backward = rearrange(
					audio_prompts_backward, 'b f c h w-> (b f) c h w'
				)
				audio_prompts_backward = rearrange(
					audio_prompts_backward, '(b f) c h w -> (b f) (c h) w', b=bsz
				)
			...
			if cfg.loss_params.mouth_gan_loss > 0:
				frames_mouth, image_pred_mouth = get_mouth_region(
					frames, image_pred, pixel_values_face_mask_backward
				)
			...
			if cfg.loss_params.gan_loss > 0:
				...
				loss_G = 0.0
				discriminator_maps_generated = loss_dict['discriminator'](
					pyramide_generated
				)
				...
				loss += (
					loss_G
					* cfg.loss_params.gan_loss
					* get_ganloss_weight(global_step)
					* adapted_weight
				)

				# Process feature matching loss if enabled
				if cfg.loss_params.fm_loss[0] > 0:
					L_feature_matching = 0.0
					for scale in loss_dict['disc_scales']:
			...
			if cfg.loss_params.mouth_gan_loss > 0:
				...
				loss_G = 0.0
				mouth_discriminator_maps_generated = loss_dict['mouth_discriminator'](
					pyramide_generated_mouth
				)
				mouth_discriminator_maps_real = loss_dict['mouth_discriminator'](
					pyramide_real_mouth
				)
				...
				loss += (
					loss_G
					* cfg.loss_params.mouth_gan_loss
					* get_ganloss_weight(global_step)
					* adapted_weight
				)
				...
				if cfg.loss_params.fm_loss[0] > 0:
					...
					for scale in loss_dict['disc_scales']:
						...
						for i, (a, b) in enumerate(
							zip(
								mouth_discriminator_maps_real[key],
								mouth_discriminator_maps_generated[key],
							)
						):
							value = torch.abs(a - b).mean()
							L_feature_matching += value * cfg.loss_params.fm_loss[i]
			...
			if cfg.loss_params.sync_loss > 0:
				pred_frames = rearrange(
					image_pred,
					'(b f) c h w-> b (f c) h w',
					f=pixel_values_backward.shape[1],
				)
			...
			if cfg.loss_params.mouth_gan_loss > 0:
				...
				avg_mouth_loss_D = accelerator.gather(
					mouth_loss_D.repeat(cfg.data.train_bs)
				).mean()
				train_loss_D_mouth += avg_mouth_loss_D.item() / 1
				mouth_loss_D = (
					mouth_loss_D * get_ganloss_weight(global_step) * adapted_weight
				)
			...
			if accelerator.sync_gradients:
				...
				if global_step % cfg.checkpointing_steps == 0:
					...
					try:
						...
						if elapsed_time > 300:
							print(
								f'Skipping storage as it took too long in step {global_step}.'
							)
						else:
							print(
								f'Resume states saved at {save_dir} successfully in {elapsed_time}s.'
							)
					except Exception as e:
		...
		if (epoch + 1) % cfg.save_model_epoch_interval == 0:
			try:
				...
				if accelerator.is_main_process:
					save_models(
						accelerator, model_dict['net'], save_dir, global_step, cfg
					)
