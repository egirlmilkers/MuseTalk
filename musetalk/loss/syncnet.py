...
def cosine_loss(a, v, y):
	...
	d = d.clamp(
		0, 1
	)  # cosine_similarity的取值范围是【-1，1】，BCE如果输入负数会报错RuntimeError: CUDA error: device-side assert triggered
...
def get_sync_loss(
	...
	frames_sync_loss = torch.cat(
		[
			gt_frames[:, : 3 * frames_left_index, ...],
			pred_frames,
			gt_frames[:, 3 * frames_right_index :, ...],
		],
		axis=1,
	)
