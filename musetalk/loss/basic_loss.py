...
if __name__ == '__main__':
	...
	loss_IN /= sum(
		cfg.loss_params.vgg_layer_weight
	)  # 对vgg不同层取均值，金字塔loss是每层叠
	print(loss_IN)

	# print(cfg.model_params.discriminator_params)

	discriminator = MultiScaleDiscriminator(**cfg.model_params.discriminator_params).to(
		device
	)
	...
	scheduler_D = CosineAnnealingLR(
		optimizer_D, T_max=cfg.discriminator_train_params.epochs, eta_min=1e-6
	)
