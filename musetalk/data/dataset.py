...
from musetalk.data.sample_method import (
	get_src_idx,
	resize_landmark,
	shift_landmarks_to_face_coordinates,
)
...
class FaceDataset(Dataset):
	...
	def __init__(self, cfg, list_paths, root_path='./dataset/', repeats=None):
		...
		for list_path, repeat_time in zip(list_paths, repeats):
			with open(list_path, 'r') as f:
				...
				for line in f.readlines():
					...
					num += 1
				print(
					f'{list_path}: {num} x {repeat_time} = {num * repeat_time} samples'
				)
	...
	def generate_random_value(self):
		...
		if self.random_margin_method == 'uniform':
			...
			random_value = np.random.normal(
				loc=self.jaw2edge_margin_mean, scale=self.jaw2edge_margin_std
			)
			...
			raise ValueError(
				f'Invalid random margin method: {self.random_margin_method}'
			)
	...
	def crop_resize_img(self, img, bbox, crop_type='crop_resize', extra_margin=None):
		...
		"""
		mask_scaled_factor = 1.0
		if crop_type == 'crop_resize':
			x1, y1, x2, y2 = bbox
			img = img.crop((x1, y1, x2, y2))
			img = img.resize((self.image_size, self.image_size), Image.LANCZOS)
		elif crop_type == 'dynamic_margin_crop_resize':
			x1, y1, x2, y2, extra_margin = self.dynamic_margin_crop(
				img, bbox, extra_margin
			)
	...
	def get_audio_file(self, wav_path, start_index):
		...
		audio_input = self.feature_extractor(
			audio_input, return_tensors='pt', sampling_rate=sampling_rate
		).input_features
	...
	def get_resized_mouth_mask(
		...
		landmark_array = np.array(
			resized_landmark[48:67]
		)  # the lip landmarks in 68 landmarks format
		...
		while attempts < self.max_attempts:
			...
			try:
				...
				if landmark_shape != (total_frames, 68, 2):
					attempts += 1
					print(
						f'video {video_path} has invalid landmark shape: {landmark_shape}, expected: {(total_frames, 68, 2)}'
					)  # we use 68 landmarks
					continue
			except Exception as e:
			...
			shift_landmarks, bbox_list_union, face_shapes = (
				shift_landmarks_to_face_coordinates(landmark_list, bbox_list)
			)
			if self.contorl_face_min_size and face_shapes[0][0] < self.min_face_size:
				print(
					f'video {video_path} has face size {face_shapes[0][0]} less than minimum required {self.min_face_size}'
				)
				attempts += 1
				continue
			...
			for src_idx in src_idx_list:
				...
				imSrc, _, _ = self.crop_resize_img(
					imSrc, bbox_s, self.crop_type, extra_margin=None
				)
				if (
					self.contorl_face_min_size
					and min(imSrc.size[0], imSrc.size[1]) < self.min_face_size
				):
			...
			if not ref_face_valid_flag:
				attempts += 1
				print(
					f'video {video_path} has reference face size smaller than minimum required {self.min_face_size}'
				)
				continue
			...
			for drive_idx in drive_idx_list:
				...
				imSameID, _, mask_scaled_factor = self.crop_resize_img(
					imSameID, bbox_s, self.crop_type, extra_margin=extra_margin
				)
				if (
					self.contorl_face_min_size
					and min(imSameID.size[0], imSameID.size[1]) < self.min_face_size
				):
				...
				if face_mask.size[1] == 0 or face_mask.size[0] == 0:
					print(
						f'video {video_path} has invalid face mask size at frame {drive_idx}'
					)
					face_mask_valid = False
					break
			...
			if not target_face_valid_flag:
				attempts += 1
				print(
					f'video {video_path} has target face size smaller than minimum required {self.min_face_size}'
				)
				continue
			...
			try:
				audio_feature, audio_offset = self.get_audio_file(
			...
			if mel.shape[0] != syncnet_mel_step_size:
				attempts += 1
				print(
					f'video {video_path} has invalid mel spectrogram shape: {mel.shape}, expected: {syncnet_mel_step_size}'
				)
				continue
...
if __name__ == '__main__':
	...
	for i in range(10):  # Check 10 samples
		...
		target_with_mask = (
			target_with_mask * (1 - target_mask) + target_mask
		)  # Apply mask only to target
		...
		concatenated_img = torch.cat(
			(
				ref_img,
				target_img,  # Original images
				torch.zeros_like(ref_img),
				target_mask,  # Mask (black for ref)
				ref_with_mask,
				target_with_mask,  # Overlay effect
			),
			dim=3,
		)
