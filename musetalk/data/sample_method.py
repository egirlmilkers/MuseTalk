...
def calculate_mouth_open_similarity(
	...
	differences_matrix = np.abs(
		mouth_open_ratios[:, np.newaxis] - mouth_open_ratios[select_idx]
	)
	differences_matrix_with_signs = (
		mouth_open_ratios[:, np.newaxis] - mouth_open_ratios[select_idx]
	)
	...
	similar_landmarks_distances = differences_matrix_with_signs[
		i
	].tolist()  # 注意这里不要排序
...
def get_closed_mouth(landmarks_list, ascending=True, top_k=50):
	...
	for i, landmarks in enumerate(landmarks_list):
		...
		mouth_top = np.array(
			landmarks[165]
		)  # Adjust index according to your landmarks format
...
def calculate_landmarks_similarity(
	selected_idx,
	landmarks_list,
	image_shapes,
	start_index,
	...
	resized_landmarks_array = np.array(
		resized_landmarks
	)  # Convert list to array for easier manipulation

	# Calculate similarity
	distances = np.linalg.norm(
		resized_landmarks_array - resized_landmarks_array[selected_idx][np.newaxis, :],
		axis=2,
	)
	overall_distances = np.mean(
		distances, axis=1
	)  # Calculate mean distance for each set of landmarks

	if ascending:
		sorted_indices = np.argsort(overall_distances)
		similar_landmarks_indices = sorted_indices[
			1 : top_k + 1
		].tolist()  # Exclude self and take top_k
	else:
		sorted_indices = np.argsort(-overall_distances)
...
def get_src_idx(drive_idx, T, sample_method, landmarks_list, image_shapes, top_k_ratio):
	...
	if sample_method == 'random':
		...
		try:
			...
			landmark_end_idx = 16
			pose_similarity_list = calculate_landmarks_similarity(
				drive_idx,
				landmarks_list,
				image_shapes,
				landmark_start_idx,
		...
		try:
			top_k = int(top_k_ratio * len(landmarks_list))
			closed_mouth_list = get_closed_mouth(
				landmarks_list, ascending=True, top_k=top_k
			)
			...
			common_list = list(
				set(closed_mouth_list).intersection(set(pose_similarity_list))
			)
		...
		try:
			...
			mouth_dissimilarity_list = calculate_landmarks_similarity(
				drive_idx,
				landmarks_list,
				image_shapes,
				landmark_start_idx,
			...
			common_list = list(
				set(pose_similarity_list).intersection(set(mouth_dissimilarity_list))
			)
			if len(common_list) == 0:
				src_idx = random.randint(drive_idx - 5 * T, drive_idx + 5 * T)
			else:
