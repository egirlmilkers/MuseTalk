...
from musetalk.loss.discriminator import DiscriminatorFullModel, MultiScaleDiscriminator
...
class Net(nn.Module):
	...
	def forward(
		...
		model_pred = self.unet(
			input_latents, timesteps, encoder_hidden_states=audio_prompts
		).sample
		return model_pred
...
def initialize_models_and_optimizers(cfg, accelerator, weight_dtype):
	...
	unet_config_file = os.path.join(
		cfg.pretrained_model_name_or_path, cfg.unet_sub_folder + '/musetalk.json'
	)
	...
	if not cfg.random_init_unet:
		pretrained_unet_path = os.path.join(
	...
	model_dict['wav2vec'] = (
		WhisperModel.from_pretrained(cfg.whisper_path)
	...
	model_dict['scheduler_max_steps'] = (
		cfg.solver.max_train_steps * cfg.solver.gradient_accumulation_steps
	)
...
def initialize_dataloaders(cfg):
	...
	dataloader_dict['train_dataset'] = PortraitDataset(
		cfg={
	...
	dataloader_dict['val_dataset'] = PortraitDataset(
		cfg={
			'image_size': cfg.data.image_size,
			'T': cfg.data.n_sample_frames,
			'sample_method': cfg.data.sample_method,
...
def initialize_loss_functions(cfg, accelerator, scheduler_max_steps):
	...
	if cfg.loss_params.gan_loss > 0:
		...
		loss_dict['scheduler_D'] = CosineAnnealingLR(
			loss_dict['optimizer_D'], T_max=scheduler_max_steps, eta_min=1e-6
		)
	...
	if cfg.loss_params.mouth_gan_loss > 0:
		...
		loss_dict['mouth_discriminator_full'] = DiscriminatorFullModel(
		...
		loss_dict['mouth_scheduler_D'] = CosineAnnealingLR(
			loss_dict['mouth_optimizer_D'], T_max=scheduler_max_steps, eta_min=1e-6
		)
...
def initialize_syncnet(cfg, accelerator, weight_dtype):
	...
	if cfg.loss_params.sync_loss > 0 or cfg.use_adapted_weight:
		...
		syncnet = SyncNet(OmegaConf.to_container(syncnet_config.model)).to(
			accelerator.device
		)
		print(
...
def initialize_vgg(cfg, accelerator):
	...
	if cfg.loss_params.vgg_loss > 0:
		vgg_IN = vgg_face.Vgg19().to(
			accelerator.device,
		)
		pyramid = vgg_face.ImagePyramide(cfg.loss_params.pyramid_scale, 3).to(
			accelerator.device
		)
		vgg_IN.eval()
