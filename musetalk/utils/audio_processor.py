	...
	def get_whisper_chunk(
		...
		for frame_index in range(num_frames):
			try:
				...
				print(
					f'num frames: {num_frames}, fps: {fps}, whisper_idx_multiplier: {whisper_idx_multiplier}'
				)
				print(
					f'frame_index: {frame_index}, audio_index: {audio_index}-{audio_index + audio_feature_length_per_frame}'
				)
				exit()
